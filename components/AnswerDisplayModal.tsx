...
const AnswerImage = ({ imageUrl, onImageClick }: { imageUrl: string, onImageClick: (url: string) => void }) => {
    const { src, isLoading } = useStoredImage(imageUrl);

    return (
        <button onClick={() => onImageClick(imageUrl)} className="w-full bg-gray-900 rounded-lg flex items-center justify-center cursor-zoom-in">
                        ...
                        {answer.imageUrls && answer.imageUrls.length > 0 && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {answer.imageUrls.map((url) => (
                                    <AnswerImage key={url} imageUrl={url} onImageClick={setZoomedImageKey} />
                                ))}
                            </div>
                        )}
                    </div>
