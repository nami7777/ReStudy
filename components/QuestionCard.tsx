...
import { Question, Difficulty, Tag } from '../types';
...
const QuestionImage = ({ imageUrl }: { imageUrl: string }) => {
    ...
    return <img src={src} alt="Question" className="w-full h-40 object-cover" loading="lazy"/>;
}


const QuestionCard = ({ question, tags, onUpdate, onDelete, onSelect, onEditAnswer, isSelected }: QuestionCardProps) => {
    ...
    return (
        <div className={`relative bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden border-b-4 ${difficultyColor[question.difficulty]} transition-all duration-300 ${isSelected ? 'ring-2 ring-indigo-500 scale-105' : 'hover:shadow-lg'}`} onClick={() => onSelect(question.id)}>
            {question.type === 'image' && question.imageUrl && <QuestionImage imageUrl={question.imageUrl} />}
            <div className="p-3">
                 <div className="h-16">
